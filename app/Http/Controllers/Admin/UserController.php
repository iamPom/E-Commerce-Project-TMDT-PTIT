...
class UserController extends Controller
{
    ...
    public function store(Request $request){
        try{
            ...
            return redirect()->route('admin.user.list', $user)->withFlashSuccess(__('Thành công'));
        }catch(Exception $e){
            session()->flash('flash_warning', 'Lỗi');
    ...
    public function updatePassword(Request $request){
        ...
        if($user!= null){
            return redirect()->route('admin.user.show')->withFlashSuccess(__('Thành công!!!!'));
        }else{
